...
class BeamSearch:
    def __init__(self, k, model, start_id, end_id, max_length, num_required=None, least_length=3):
        '''
            Args:
              k: Beam search size
              ...
              max_length: Maximum length of ids
        ...
        self.least_length = least_length + 2
    ...
    def _step(self, model_inp):
        ...
        for i, leaf_node in enumerate(self.leaf_nodes):
            ...
            if leaf_node.token_id == self.end_id and leaf_node.length > self.least_length:
                self.end_nodes.append(leaf_node)
                
            # Predict log prob
            model_inp['tar'] = torch.tensor(leaf_node.get_ids(), dtype=torch.int64).unsqueeze(0)
            ...
            for value, token_id in zip(value_top_k, argtop_k):
                ...
                if index is not None:
                    ...
                    next_leaf_nodes.insert(index, BeamNode(token_id, leaf_node, log_prob, leaf_node.length + 1))
    ...
    def __call__(self, model_inp):
        ...
        for i in range(self.max_length - 1):
            ...
            if len(self.end_nodes) >= self.num_required:
                break
            print('step: {}, end_nodes: {}'.format(self.count, len(self.end_nodes)))
